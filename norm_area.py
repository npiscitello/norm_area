...
# Uses Riemann sums - automatically determines how many
# rectangles to use by trying more and more until the 
# left and right sums agree to the correct number of
# decimal places (halfup rounded), then outputs that number.
...
NUM_RECTS_INIT = 1	# Initial number of rectangles...gotta start somewhere!
NUM_RECTS_SCALE = 1.15	# the multiplier of num_rects every iteration
ASK_THRESH = 1000000	# how many rectangles needed to check in with the user

	# handle command line options for the above constants
try:
	...
	sys.exit('\nusage: python norm_area.py  A  B  Mean  SD  -i <init # rectangles>'\
	' -s <rect # scale> -t <user alert threshold>\n')
...
for opt, arg in opts:
	...
	# test that there are the right number of arguments
if len(sys.argv) < 6:
	sys.exit('\n5 Arguments Required! Format: A  B  Mean  SD  #DPs\n')

	# test that the arguments are of the correct types
try:
...
right = 1.1;	left = 0.1	# assign values to force while loop to execute
num_rects = NUM_RECTS_INIT;	asked = 0
while round(right,dec_pts) != round(left,dec_pts):
